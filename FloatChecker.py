    ...
    def __init__(self, items):
        ...
        with open('proxies_for_float.txt', 'r', encoding='utf-8') as file:
            ...
            for line in file.readlines():
                proxy = 'http://' + line.strip().split(':')[2] + ':' + line.strip().split(':')[3] + '@' + \
        ...
        if item['fl'] != '1':
            ...
            if is_rarity(skin, fl, percent):
                ...
                self.result_thread[idx] = (
                    ...
                    sticker, url,
                    str(ts), str(wear), str(sticker_slot), str(sticker_price), str(profit), str(percent), str(fl), page_num)
        else:
            time.sleep(2.5)
